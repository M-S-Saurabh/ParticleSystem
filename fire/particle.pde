class Particle{
  ...
  void display(){
    stroke(splColor);
    fill(splColor);
    ellipse(location.x,location.y,size*1.1,size);
  }
  void restart(PVector l){
    acceleration = new PVector(0.0, 0.0);
    velocity = new PVector(random(-0.5,0.5), random(-1.5,-0.5));
    location = l.copy();
    origin = l.copy();
    lifespan = 50.0;
    alpha = 255.0;
...
class SmokeParticle extends Particle{
  ...
  @Override
  void restart(PVector l){
    acceleration = new PVector(0.0, random(-0.1,0.0));
    velocity = new PVector(0.0, random(-0.5,-0.0));
    location = l.copy();
    origin = l.copy();
    lifespan = 100.0;
